...
def validate_synthetic_quality(synthetic_imgs: List[Image.Image], syn_feats: np.ndarray, fid: float) -> Tuple[bool, str]:
    """
    Quality Validation Guard: Checks whether synthetic dataset is valid structure or garbage/noise.
    """
    if len(synthetic_imgs) == 0 or len(syn_feats) == 0:
        return False, "Synthetic image dataset is empty."
        
    # Check feature std deviation across samples
    feat_std = float(syn_feats.std(axis=0).mean()) if len(syn_feats) > 1 else 0.0
    
    # Extreme FID (> 350) indicates random noise / complete distribution mismatch
    if fid > 350.0:
...
def evaluate_model_synthetic_dataset(session_id: str, model_type: str) -> Dict[str, Any]:
    """
    Evaluates synthetic dataset for model_type ('gan', 'diffusion', 'vae_gan') against session's real preprocessed dataset.
    Includes Quality Validation Guard step.
    """
    ...
    is_valid, validation_msg = validate_synthetic_quality(synthetic_imgs, syn_feats, fid)
    ...
    if not is_valid:
        ...
        # Quality Score: lower FID & higher SSIM/hist_sim = higher score (0-100)
        fid_score_component = max(0.0, 100.0 - (fid * 1.5))
        ssim_component = max(0.0, ssim * 100.0)
        hist_component = max(0.0, hist_sim * 100.0)
        quality_score = float(np.clip(0.5 * fid_score_component + 0.3 * ssim_component + 0.2 * hist_component, 0.0, 100.0))
